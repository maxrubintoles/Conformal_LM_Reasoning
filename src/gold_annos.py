...
def topological_sort(adj_list, subgraph):
    ...
    queue = deque([node for node in in_degree if in_degree[node] == 0 and subgraph[node] == 1])
    top_order = []
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python3 -m src.gold_annos <first_question_index> <last_question_index>")
        return

    first_question_index = int(sys.argv[1])
    last_question_index = int(sys.argv[2])

    file_name = input("Enter the file name (e.g., MATH_open_subclaims_with_scores.json): ")
    ...
    for question_index in range(first_question_index, last_question_index + 1):
        ...
        for subgraph in legal_subgraphs:
            ...
            sorted_subclaims = [question['claims'][i]["subclaim"] for i in sorted_subgraph_indices]
            ...
            while True:
                user_input = input("Is this a coherently factual output? (Y/N): ").strip().upper()
