...
from src.helpers.dependent_cp.simult import simult_search, topological_sort, to_valid_array
import json
...
def manual_anno(subgraph, question, filepath="/Users/maxonrubin-toles/Desktop/Conformal/Open_Source/data/MATH_subclaims_with_scores.json", calib_method = "graph"):
    ...
    if idxs in question["graph_annotations"]["y"]:
        ...
        if updated:
            try:
                ...
                print(f"Updated annotations saved to {filepath}.")
            except IOError as e:
...
def graph_anno(subgraph, question):
    ...
    roots = [node for node in range(len(dep_graph)) if all(dep_graph[node][i] == 0 for i in range(len(dep_graph)))]
...
def annotate(subclaims, question, type):
    if type == "ind":
        return ind_anno(subclaims, question)
    elif type == "graph":
        return graph_anno(subclaims, question)
    elif type == "manual":
        return manual_anno(subclaims, question)
    '''
...
'''
def r_score(question, noise, method = "simult", anno_type = "manual", beta = None):
    if method == 'ind_filtered':
        method = 'ind'
    if method == "simult":
        U = simult_search(question, noise, beta = beta)
    ...
    if anno_type == "manual":
        ...
        for i in range(len(U)):
            ...
            if not manual_anno(vertices, question):
                return risk
